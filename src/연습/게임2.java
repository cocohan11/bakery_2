...
public class 게임2 extends Thread{         //쓰레드라서 상속해줌
...
/*작업실();*//*
    ...
    private void keyProcess() {                 // 화면에서 사장이 안나가는 선에서 사장값 조정.
        ...
        if (down && 사장Y + 사장Height + 사장Speed < 메.SCREEN_HEIGHT) 사장Y += 사장Speed;
        if (left && 사장X - 사장Speed > 0) 사장X -= 사장Speed;
        if (right && 사장X + 사장Width - 사장Speed*2 < 메.SCREEN_WIDTH) 사장X += 사장Speed;
    ...
    public void 작업실() {
        ...
        if (사장X + 사장Width + 사장Speed >= 메.SCREEN_WIDTH && 사장Y + 사장Height >= 작업문_오.y+50) {
            // +30은 미세조정
...
/*            isBakery3 = false;
        ...
        if (밀.x <= 사장X && 밀.x끝 >= 사장X) {
            ...
            동전먹기 동 = new 동전먹기();                         // 미니게임 불러오기    // 시간제한 두고싶음


            밀.현재고 += 동.score;                   // 동전먹기로 밀가루 가져오기   // 문제) 화면이 안 꺼짐
        }
