...
public class 베이커리게임2 extends JFrame {        // JFrame 상속한다. // JFrame이란? 창이다.
    ...
    private Image 작업실 = new ImageIcon("src/이미지/작업실.jpg").getImage();          // getImage()를 해야 이미지를 불러오나 봄(?)
    ...
    public 베이커리게임2() {                       // 생성자 (메인에서 호출할거임)

        setTitle("베이커리 게임");
        */
/*setUndecorated(true);    << 이거 x버튼 없어서 불편해*//*

        setSize(메인.SCREEN_WIDTH,메인.SCREEN_HEIGHT);         // 창 크기
...
/*    public void 작업실() {
    ...
    public void paint(Graphics g) {             // paint라는 메소드는 JFrame의 메소드인가봄. 내가 변경할 수 있는 이름이 아님
        ...
        bufferImage = createImage(메인.SCREEN_WIDTH, 메인.SCREEN_HEIGHT);       // 이미 생성자가 정해져있네. 가로세로  // 위의 변수에 값 넣음
